...
O Factory Method é um padrão criacional de projeto que fornece uma interface para criar objetos, mas deixa que as subclasses decidirem qual classe instanciar. 
...
Tomando como base o exemplo aplicado no [Single Factory Pattern](). Digamos que a aplicação começa a crescer e agora a `FanFactory` precisa que fazer um novo tipo de ventilador chamado `WallFan`. 
...
O código é o mesmo usado no exemplo [Single Factory Pattern]().
...
A complexidade aumentou. Para criar três tipos de objetos (TableFan, CeilingFan e ExhaustFan), tivemos de criar três classes Factories, enquanto no [Simple Factory Pattern]() criamos apenas uma Factory. 

Agora surge a pergunta: por que precisamos usar o Factory Method Pattern, já que podemos conseguir o mesmo resultado com o [Simple Factory Pattern]()? 

Simples. Na [definição do problema]() foi mencionando um dos motivos: suponha que a empresa vá lançar um novo ventilador chamado `WallFan`. Este é um novo requisito. 
Usando o Factory Method Pattern, **não adicionamos novas condições no `switch case`** como pode ser visto no [Simple Factory Pattern]() (vide [Prós e contras do Factory Method Pattern]()). 
...
| Escrever casos de teste de unidade é fácil com o Factory Method Pattern em comparação com o [Simple Factory Pattern](), uma vez que o `switch case` (ou blocos longos de if-else) não é usado. | |
...
Entendemos o uso do Factory Method Pattern com um passo a passo. Compreendemos o contexto do Factory Method Pattern e como ele é diferente do [Simple Factory Pattern](). 
